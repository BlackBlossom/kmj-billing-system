...
export const validateRegistration = [
  ...
  body('ward')
    ...
    .matches(/^[0-9]+$/).withMessage('Ward must be a number'),
...
export const validateLogin = [
  body('memberId')
    ...
    .matches(/^[\w]+\/[\w]+$/).withMessage('Member ID must be in format: ward/house (e.g., 1/25 or ADMIN/001)'),
...
export const validateMemberCensus = [
  body('Mid')
    .trim()
    .notEmpty().withMessage('Mahal ID is required')
    .matches(/^(\d+\/\d+|TEMP\/.+)$/).withMessage('Invalid Mahal ID format'),
  ...
  body('Aadhaar')
    .optional({ checkFalsy: true })
    .matches(/^[0-9]{12}$/).withMessage('Aadhaar must be 12 digits'),
  
  body('Myear')
    .optional({ checkFalsy: true })
    .matches(/^[0-9]{4}$/).withMessage('Year must be 4 digits'),
...
export const validateBillCreation = [
  ...
  body('id_name_address')
    .trim()
    .notEmpty().withMessage('Member details are required'),
...
export const validateAccountEntry = [
  ...
  body('Tariff')
    .notEmpty().withMessage('Tariff amount is required')
    .isFloat({ min: 0 }).withMessage('Tariff must be a valid number')
    .toFloat(),
...
export const validateSearch = [
  ...
  query('field')
    .optional()
    .trim()
    .isIn(['name', 'memberId', 'aadhaar', 'phone', 'all']).withMessage('Invalid search field'),
...
export const validateNoticeCreation = [
  ...
  body('priority')
    .optional()
    .isIn(['urgent', 'high', 'normal', 'low']).withMessage('Priority must be urgent, high, normal, or low'),
